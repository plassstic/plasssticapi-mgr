...
func (m *UserMutation) SetEditable(s schema.Editable) {
	m.editable = &s
}

// Editable returns the value of the "editable" field in the mutation.
func (m *UserMutation) Editable() (r schema.Editable, exists bool) {
	v := m.editable
	if v == nil {
		return
	}
...
// OldEditable returns the old "editable" field's value of the User entity.
// If the User object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *UserMutation) OldEditable(ctx context.Context) (v schema.Editable, err error) {
	if !m.op.Is(OpUpdateOne) {
